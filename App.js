...
function App() {
  ...
  const contractAddress = "0xe1175B7CD793B60B1B1c50CF4132aB906067B8f9";
  ...
  const registerPerson = async () => {
    ...
    try {
      ...
      await loadPeople();
      await loadExpenses();
      await loadTotalUsers(); //  Ensure updated count after register
    } catch (error) {
      ...
      <header className="App-header">
        ...
        {!isConnected ? (
          ...
          <div>
            <h2>Welcome, {name}</h2>
            <p>Account: {account}</p>
            <p>Total Registered Users: {totalUsers}</p> {/* DISPLAY */}
